...
use crate::producers::builder::{Builder, BuildGate};
use BuildGate::*;
...
fn add_lc(b: &mut Builder, lc: &Vec<zkiVariable>) -> WireId {
    if lc.len() == 0 {
...
fn build_term(b: &mut Builder, term: &zkiVariable) -> WireId {
    if term.id == 0 {
...
pub fn to_ir(
    ...
    let mut b = Builder::new(MemorySink::default(), header);
