...
pub enum BuildGate {
    ...
    And(FieldId, WireId, WireId),
    Xor(FieldId, WireId, WireId),
    Not(FieldId, WireId),
...
impl BuildGate {
    pub fn with_output(self, output: WireId) -> Gate {
        match self {
            ...
            And(field, left, right) => Gate::And(field, output, left, right),
            Xor(field, left, right) => Gate::Xor(field, output, left, right),
            Not(field, input) => Gate::Not(field, output, input),
