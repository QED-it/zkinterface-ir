...
/// This function performs the following checks on zkif_vector_add/mul inputs.
...
/// - `output_count` and `input_count` are compliant with `plugin(zkif_vector, add/mul, params)`
/// - `inputs` is compliant with `plugin(zkif_vector, add/mul, params)`
fn zkif_vector_check<'a>(
    ...
    if !public_inputs.is_empty() {
        return Err("plugin(zkif_vector, add/mul) does not consume any public input.".into());
    }
    if !private_inputs.is_empty() {
        return Err("plugin(zkif_vector, add/mul) does not consume any private input.".into());
    }

    // Check that params are compliant with the plugin zkif_vector and the operation add/mul
    if params.len() != 2 {
        return Err(
    ...
    let type_ = types.get(param_type_id as usize).ok_or_else(|| {
        format!(
            "plugin(zkif_vector, add/mul) cannot be called with a type id ({}) which is not defined.",
            param_type_id
        )
    })?;
    let modulo = match type_ {
        ...
        PlaintextType::PluginType(_, _, _) => {
            return Err("plugin(zkif_vector, add/mul) cannot be called on a PluginType.".into())
        }
    };

    // Check that `output_count` and `input_count` are compliant with `plugin(zkif_vector, add/mul, params)`
    let expected_output_count = vec![Count::new(param_type_id, u64::try_from(param_len)?)];
    if *output_count != expected_output_count {
        return Err(format!(
            "When calling the plugin(zkif_vector, add/mul, {}, {}), the out parameter in the function signature must be equal to {:?} (and not {:?}).",
    ...
    if *input_count != expected_input_count {
        return Err(format!(
            "When calling the plugin(zkif_vector, add/mul, {}, {}), the in parameter in the function signature must be equal to {:?} (and not {:?}).",
            param_type_id, param_len, expected_input_count, input_count
        )
            .into());
    }

    // Check that `inputs` is compliant with `plugin(zkif_vector, add/mul, params)`
    if inputs.len() != 2 * param_len {
        return Err(format!(
            "When calling the plugin(zkif_vector, add/mul, {}, {}), we should have {} input values (and not {}).",
...
/// @function(vector_add, @out: type_id: length, @in: type_id: length, type_id: length) @plugin(zkif_vector, add, type_id, length)
...
pub fn zkif_vector_add(
    ...
    let (param_len, modulo) = zkif_vector_check(
        output_count,
        input_count,
        inputs,
        public_inputs,
...
/// @function(vector_mul, @out: type_id: length, @in: type_id: length, type_id: length) @plugin(zkif_vector, mul, type_id, length)
/// This function takes as input two vectors `in1` and `in2` of length `length` containing elements from type `type_id`,
/// This function returns one vector of length `length` such that `out[i] = in1[i] * in2[i] % type_modulo`
pub fn zkif_vector_mul(
    output_count: &[Count],
    input_count: &[Count],
    inputs: &[&BigUint],
    public_inputs: &BTreeMap<TypeId, Vec<BigUint>>,
    ...
    // Evaluate plugin(zkif_vector, mul)
    let mut result = vec![];
    for i in 0..param_len {
        result.push((inputs[i] * inputs[i + param_len]) % modulo);
    }
...
fn test_zkif_vector_check() {
    ...
    // Try to use the plugin zkif_vector with an unknown type_id in params
    ...
    // Try to use the plugin zkif_vector with a type_id which cannot be parsed into u8
    ...
    // Try to use the plugin zkif_vector with a param_len equal to 0
    ...
    // Try to use the plugin zkif_vector with an incorrect output_count
    ...
    // Try to use the plugin zkif_vector with an incorrect number of inputs
...
fn test_vector_add() {
    ...
    // Try to use the plugin(zkif_vector, add, params) with 3 parameters instead of 2
    let incorrect_params = ["1".to_string(), "2".to_string()];
    let result = zkif_vector_add(
        &output_count,
        &input_count,
