...
impl Source {
    ...
    pub fn from_buffers(buffers: Vec<Vec<u8>>) -> Self {
        Source {
            buffer_source: BufferSource::Memory(buffers),
            print_filenames: false,
        }
    }
    ...
    pub fn iter_buffers<'w>(&'w self) -> Box<dyn Iterator<Item = Vec<u8>> + 'w> {
        match &self.buffer_source {
            ...
            BufferSource::Files(paths) => Box::new(iterate_files(&paths[..], self.print_filenames)),
            BufferSource::Memory(buffers) => Box::new(iterate_buffers(&buffers[..])),
        }
    }
...
pub fn iterate_file(path: &Path) -> Box<dyn Iterator<Item = Vec<u8>>> {
    match File::open(path) {
        ...
        Ok(file) => Box::new(iterate_stream(file)),
    }
}
...
pub fn iterate_stream<'s>(mut stream: impl Read + 's) -> impl Iterator<Item = Vec<u8>> + 's {
    iter::from_fn(move || match read_buffer(&mut stream) {
        Err(err) => {
...
pub fn list_workspace_files(paths: &[PathBuf]) -> Result<Vec<PathBuf>> {
    ...
    for path in paths {
        if has_sieve_extension(path) {
            ...
            if paths.len() > 1 {
                return Err("Cannot combine files and stdin".into());
            }
            all_paths.push(path.clone());
        } else {
